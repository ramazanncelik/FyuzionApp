...
import TargetUserMessageMenu from './Menu';
import FastImage from 'react-native-fast-image';
import { language } from '../../../utils/utils';
import { useAuthContext } from '../../../navigation/AuthProvider';

const TargetUserMessage = ({ message }) => {
    ...
    if (!isMenuVisible) {
        if (message.Type === "text") {
            return (
                <View className="w-full flex flex-row items-center justify-start my-1">
                    ...
                    <TouchableOpacity onLongPress={() => setIsMenuVisible(true)} className="w-[75%] flex flex-col px-2 py-1 bg-orange-500 rounded-lg">
                        ...
                        <View className="w-full flex flex-row space-x-2">
                            ...
                            <Text className="flex flex-1 text-sm text-right text-gray-200">
                                {date}
                            </Text>
                        </View>
                    </TouchableOpacity>
            ...
            return (
                <View className="w-full flex flex-row items-center justify-start my-1">
                    ...
                    <TouchableOpacity onLongPress={() => setIsMenuVisible(true)} className={`w-[75%] flex flex-col px-2 py-1 rounded-lg border border-orange-500`}>
                        <Text className="text-white font-bold">
                            {message.fromUser.NickName}
                        </Text>
                        ...
                        <Text className="text-sm text-right text-gray-200">
                            {date}
                        </Text>
                    </TouchableOpacity>
        ...
        return (
            <TargetUserMessageMenu message={message} setIsMenuVisible={setIsMenuVisible} />
        )
    }
}
